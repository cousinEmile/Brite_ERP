...
public class BRIT_254 extends TestBase {


    @Test
    public void AdvancedSearchQueries_GroupBy_Privacy(){
        extentLogger = report.createTest("Advanced Search Queries : GroupBy : Privacy");
        ...
        extentLogger.info("select 'Privacy' from the 'Group By' drop down menu.");
        extentLogger.info("Expected Results: User should observe a new tag in the Search Bar labeled 'Privacy'. ");
        pages.searchPage().groupPrivacy_button.click();
        Assert.assertTrue(pages.searchPage().Privacy_SearchBarIcon.isDisplayed());
        Assert.assertEquals(pages.searchPage().Privacy_Label_SearchBarIcon.getAttribute("innerText").trim(),PRIVACY_SEARCHBARICON_LABEL);

        extentLogger.pass("BRIT_237 AdvancedSearchQueries_GroupBy_Privacy Test PASS");
    }
